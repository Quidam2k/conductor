...
In real use, participants lock their phones and put them away — eyes on their surroundings, hands free. When an event goes live, Conductor pre-renders ("bakes") every beep and resource-pack voice cue into one continuous audio track, so those cues keep playing with the screen off. Spoken text (text-to-speech) cannot be baked — it only plays while the screen is on.

What a participant actually hears:

| Cue | Screen on | Locked / pocketed |
|-----|-----------|-------------------|
...
- **Real events want a resource pack.** Pack voice cues are the only way participants hear *words* from a pocket. If your actions reference pack cues, the pocketed experience is fully spoken. You can record cues in your own voice using the 🎤 buttons while editing actions — the editor lets you build a personal voice pack and share it with your group.
- **No pack? Design for beeps alone.** Every action still gets its countdown beeps and a trigger beep under lock. Keep the choreography simple enough that "beeps counting down, then the go beep" carries it — participants learn what each moment means from the briefing, not from a voice.
- **Briefing blocks matter more, not less.** The briefing is read before phones go into pockets. Put the action sequence, roles, and signals there (see below).
- Countdown and trigger beeps are generated by the app — a resource pack cannot replace them (see `RESOURCE_PACK_FORMAT.md`).

The bundled demo scripts are format showcases — they lean on spoken text so they work with nothing installed. Treat them as syntax examples, not as templates for a real pocketed event.
