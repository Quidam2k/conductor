...
Countdowns are generated by the app itself as ascending tonal beeps, not played from pack audio — **a pack cannot override or replace them**. At the trigger moment, the action's own cue plays (or TTS speaks the action text as fallback) — there is no separate "Now!" cue.

Legacy system cue IDs (`countdown-voice`, `countdown-5` through `countdown-1`, `trigger`) are no longer consulted by the audio service. Packs may still include them; they are ignored.

**Why packs matter for real events:** when an event goes live, beeps and pack voice cues are baked into one continuous audio track that keeps playing while the phone is locked and pocketed. Text-to-speech cannot be baked — it only plays screen-on. Pack voice cues are therefore THE way participants hear spoken cues from a pocket; without a pack, a locked phone plays beeps only.
