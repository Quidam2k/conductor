...
Countdowns are generated by the app itself as ascending tonal beeps (Web Audio), not played from pack audio. At the trigger moment, the action's own cue plays (or TTS speaks the action text as fallback) — there is no separate "Now!" cue.
